...
public class OrgHealthBaselineJob implements Job {
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        log.info("🚀 开始执行组织健康基线生成作业: {}", jobName);
        
        try {
            long startTime = System.currentTimeMillis();
            
            // 调用HealthBaselineScoreTasks中的组织基线生成方法
            healthBaselineScoreTasks.generateOrgHealthBaseline();
            
            long executionTime = System.currentTimeMillis() - startTime;
            log.info("✅ 组织健康基线生成作业完成: {}, 耗时: {}ms", jobName, executionTime);
            
        } catch (Exception e) {
            log.error("❌ 组织健康基线生成作业失败: {}, 错误: {}", jobName, e.getMessage(), e);
            throw new JobExecutionException("组织健康基线生成失败: " + e.getMessage(), e);
        }
    }
}