...
public interface UserHealthProfileMapper extends BaseMapper<UserHealthProfile> {
     ...
     * 查询用户最新的健康画像
     */
    @Select("SELECT * FROM t_health_profile WHERE user_id = #{userId} AND is_deleted = 0 " +
     ...
     * 查询指定日期的用户健康画像
     */
    @Select("SELECT * FROM t_health_profile WHERE user_id = #{userId} AND profile_date = #{profileDate} " +
     ...
     * 查询用户画像历史
     */
    @Select("SELECT * FROM t_health_profile WHERE user_id = #{userId} " +
     ...
     * 查询健康等级分布统计
     */
    @Select("SELECT health_level, COUNT(*) as count FROM t_health_profile " +
     ...
     * 查询风险等级分布统计
     */
    @Select("SELECT current_risk_level, COUNT(*) as count FROM t_health_profile " +
            "WHERE customer_id = #{customerId} AND profile_date = #{profileDate} AND is_deleted = 0 " +
