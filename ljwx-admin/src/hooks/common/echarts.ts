  ...
  async function render() {
    try {
      if (!isRendered()) {
        ...
        // 修复：多重检查domRef.value是否存在
        if (!domRef.value) {
          console.warn('domRef is null, cannot initialize chart');
          return;
        }

        // 修复：检查DOM元素是否已被挂载
        if (!domRef.value.isConnected) {
          console.warn('domRef element is not connected to DOM');
          return;
        }
        ...
        if (rect.width === 0 || rect.height === 0) {
          console.warn('domRef element has zero dimensions:', rect);
          return;
        }
