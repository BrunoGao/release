...
<template>
    ...
    <NCard title="客户选择" class="h-full shadow-sm">
      <div class="space-y-6">
        ...
        <div class="border border-blue-200 rounded-lg bg-blue-50 p-4">
          <h3 class="mb-2 text-lg text-blue-900 font-medium">当前选中客户</h3>
          <div v-if="currentCustomer" class="text-blue-700">
            <p>
              <span class="font-medium">客户ID:</span>
          ...
          <div v-else class="text-gray-500">未选择客户</div>
        </div>

        <!-- API 请求信息 -->
        <div class="border border-green-200 rounded-lg bg-green-50 p-4">
          <h3 class="mb-2 text-lg text-green-900 font-medium">API 请求参数</h3>
          <div class="text-green-700">
            <p>
              <span class="font-medium">当前用户角色:</span>
              {{ userInfo?.userName || 'N/A' }}
            </p>
            <p>
              <span class="font-medium">用户原始customerId:</span>
              {{ userInfo?.customerId || 'N/A' }}
            </p>
            <p>
              <span class="font-medium">选择器选中customerId:</span>
              {{ customerStore.currentCustomerId || 'N/A' }}
            </p>
            <p>
              <span class="font-medium">实际使用customerId:</span>
              {{ getEffectiveCustomerId() }}
            </p>
            <p class="mt-2 text-sm text-green-600">说明：{{ getCustomerIdExplanation() }}</p>
          </div>
        </div>

        <!-- 客户选择器 -->
        <div class="space-y-4">
          <h3 class="text-lg text-gray-900 font-medium">选择客户</h3>
          <div class="flex items-center space-x-4">
            <label class="whitespace-nowrap text-sm text-gray-700 font-medium">客户:</label>
            <div class="max-w-md flex-1">
              <CustomerSelector ref="customerSelectorRef" @change="handleCustomerChange" />
            </div>
          </div>
        </div>
 ...
 操作说明 */
        ...
        <!-- 调试信息 
-->
        <div v-if="showDebug" class="border border-yellow-200 rounded-lg bg-yellow-50 p-4">
          <h3 class="mb-2 text-lg text-yellow-900 font-medium">调试信息</h3>
          <pre class="overflow-auto text-xs text-yellow-700">{{ debugInfo }}</pre>
        </div>
      </div>
    </NCard>
  </div>
