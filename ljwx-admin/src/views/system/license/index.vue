<template>
  <div class="license-management">
      ...
      <div class="header-content">
        ...
        <div class="action-buttons">
          ...
          <NButton 
            ...
            v-if="hasPermission('license:management:import')"
          >
            <template #icon>
      ...
      <NCard title="📋 License 状态" class="status-card">
        <div class="license-status">
          <div class="status-indicator">
            <NBadge 
              :type="systemStatus.licenseValid ? 'success' : 'error'"
              :text="systemStatus.licenseValid ? '有效' : '无效'"
            />
          ...
          <div class="license-info" v-if="systemStatus.licenseInfo">
            <NDescriptions :column="2" bordered>
              ...
              <NDescriptionsItem label="剩余天数">
                <NBadge 
                  :type="getRemainingDaysType(systemStatus.remainingDays)"
                  :text="systemStatus.remainingDays + ' 天'"
                />
              </NDescriptionsItem>
            </NDescriptions>
          </div>
          ...
          <div class="license-actions">
            <NButton 
              type="warning" 
              @click="revalidateLicense"
              :loading="revalidating"
              v-if="hasPermission('license:management:revalidate')"
            >
              重新验证
            </NButton>
            <NButton 
        ...
        >
          <NUploadDragger>
            ...
            <NP depth="3" style="margin: 8px 0 0 0">
              支持.lic格式的License文件，文件大小不超过10MB
            </NP>
          </NUploadDragger>
        </NUpload>
      </div>
    ...
    <!-- 租户License状态弹窗 -->
    <NModal
      v-model:show="showTenantStatusModal"
      preset="card"
      title="租户License状态"
      style="width: 600px;"
      @after-leave="selectedTenant = null"
    >
      <div v-if="selectedTenant">
        <NDescriptions :column="1" bordered>
          ...
          <NDescriptionsItem label="License支持">
            <NBadge 
              :type="selectedTenant.supportLicense ? 'success' : 'error'"
              :text="selectedTenant.supportLicense ? '已启用' : '未启用'"
            />
          </NDescriptionsItem>
          ...
          <NDescriptionsItem label="错误信息" v-if="selectedTenant.licenseStatus?.error">
...
import { 
  ...
  NDataTable, NInput, NBadge, NDescriptions, NDescriptionsItem,
  useMessage, useDialog
} from 'naive-ui'
import { 
  RefreshOutline, CloudUploadOutline, SearchOutline, 
    ...
    onPositiveClick: async () => {
      try {
        ...
        const response = await fetch(
          `/api/license/management/tenant/${tenant.customerId}/toggle/${enabled}`,
          { method: 'POST' }
        )
        const data = await response.json()
