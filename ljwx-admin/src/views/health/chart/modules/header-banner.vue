...
onMounted(async () => {
  try {
    ...
    if (response.data && response.data.lives && response.data.lives.length > 0) {
      ...
      weatherDesc.value = $t('page.health.chart.weatherUnavailable');
    }
  } catch (error) {
    weatherDesc.value = $t('page.health.chart.weatherError');
  }
});
</script>
      ...
      <NGi span="24 s:24 m:18">
        <div class="flex-y-center">
          <div class="size-72px shrink-0 overflow-hidden rd-1/2">
            <img src="@/assets/imgs/health.jpg" class="size-full" />
          </div>
          <div class="pl-12px">
