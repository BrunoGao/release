<template>
  <div class="flex-vertical-stretch gap-16px overflow-hidden <sm:overflow-auto">
    <NCard title="设备绑定申请管理" :bordered="false" size="small" class="sm:flex-1-hidden card-wrapper">
      ...
      <!-- 批量操作 -->
      <div class="flex-y-center gap-12px mb-16px">
        <NButton 
          type="primary" 
          :disabled="!checkedRowKeys.length"
          @click="batchApprove('APPROVED')"
        >
          <template #icon>
...
import { NButton, NTag, NPopconfirm, NModal, NCard, NForm, NFormItem, NInput, NSelect } from 'naive-ui';
import type { DataTableColumns, FormInst, FormRules } from 'naive-ui';
import { fetchDeviceBindingApplications, batchApproveBindingApplications } from '@/service/api';
...
async function performApprove() {
  try {
    ...
    $message.success(
      currentApproveAction === 'APPROVED' 
        ? `成功通过 ${currentApproveIds.length} 个申请`
        : `成功拒绝 ${currentApproveIds.length} 个申请`
    );
