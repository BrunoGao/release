...
const model = defineModel<Api.Customer.CustomerConfigSearchParams>('model', { required: true });
        ...
        <NGridItem span="4">
          <NGrid responsive="screen" item-responsive :x-gap="8">
            <NFormItemGi span="24 s:8 m:12" :label="$t('page.customer.config.customerName')" path="departmentName">
              <NSelect v-model:value="model.id" size="small" :placeholder="$t('page.customer.config.customerName')" :options="props.orgUnitsName" />
            </NFormItemGi>
          </NGrid>
        </NGridItem>
        <NGridItem>
