#!/usr/bin/env python3
"""批量模拟健康数据上传脚本（高效版本）"""
import requests,json,random,time,sys,os
from datetime import datetime,timedelta
import mysql.connector
from concurrent.futures import ThreadPoolExecutor,as_completed

# 配置
API_URL = "http://localhost:5001/upload_health_data"
BASELINE_API = "http://localhost:5001/api/baseline/generate"
DB_CONFIG = {'host':'127.0.0.1','user':'root','password':'123456','database':'lj-06','port':3306}

class HealthDataSimulator:
    def __init__(self):
        self.session = requests.Session()  # 复用连接
        self.success_count = 0
        self.error_count = 0
    
    def get_users(self, limit=None):
        """获取用户列表"""
        try:
            conn = mysql.connector.connect(**DB_CONFIG)
            cursor = conn.cursor(dictionary=True)
            sql = "SELECT id,user_name,device_sn FROM sys_user WHERE device_sn IS NOT NULL AND device_sn != '' AND device_sn != '-' AND is_deleted = 0"
            if limit:
                sql += f" LIMIT {limit}"
            cursor.execute(sql)
            users = cursor.fetchall()
            cursor.close()
            conn.close()
            return users
        except Exception as e:
            print(f"获取用户失败: {e}")
            return []
    
    def generate_batch_data(self, device_sn, start_time, hours=24):
        """生成批量数据（一天的数据）"""
        batch_data = []
        current_time = start_time
        end_time = start_time + timedelta(hours=hours)
        
        while current_time < end_time:
            data = {
                'id': device_sn,
                'upload_method': 'wifi',
                'heart_rate': random.randint(60, 120),
                'blood_oxygen': random.randint(95, 100),
                'body_temperature': f"{random.uniform(36.0, 37.5):.1f}",
                'blood_pressure_systolic': random.randint(90, 140),
                'blood_pressure_diastolic': random.randint(60, 90),
                'step': random.randint(500, 2000),
                'distance': f"{random.uniform(300, 1500):.1f}",
                'calorie': f"{random.uniform(20000, 60000):.1f}",
                'latitude': f"{random.uniform(22.5, 22.6):.14f}",
                'longitude': f"{random.uniform(114.0, 114.1):.14f}",
                'altitude': '0.0',
                'stress': random.randint(20, 80),
                'timestamp': current_time.strftime('%Y-%m-%d %H:%M:%S'),
                'sleepData': '{"code":0,"data":[{"endTimeStamp":1747440420000,"startTimeStamp":1747418280000,"type":2}],"name":"sleep","type":"history"}',
                'exerciseDailyData': '{"code":0,"data":[{"strengthTimes":2,"totalTime":5}],"name":"daily","type":"history"}',
                'exerciseWeekData': 'null',
                'scientificSleepData': 'null',
                'workoutData': '{"code":0,"data":[],"name":"workout","type":"history"}'
            }
            batch_data.append(data)
            current_time += timedelta(minutes=5)
        
        return batch_data
    
    def upload_batch(self, batch_data):
        """批量上传数据"""
        try:
            payload = {'data': batch_data}
            response = self.session.post(API_URL, json=payload, timeout=30)
            if response.status_code == 200:
                self.success_count += len(batch_data)
                return True
            else:
                self.error_count += len(batch_data)
                print(f"批量上传失败: {response.status_code}")
                return False
        except Exception as e:
            self.error_count += len(batch_data)
            print(f"批量上传异常: {e}")
            return False
    
    def simulate_user_month(self, user, days=30):
        """模拟用户一个月的数据"""
        device_sn = user['device_sn']
        user_name = user['user_name']
        
        end_date = datetime.now()
        start_date = end_date - timedelta(days=days)
        
        success_days = 0
        current_date = start_date
        
        while current_date < end_date:
            # 生成一天的数据
            batch_data = self.generate_batch_data(device_sn, current_date, 24)
            
            # 上传批量数据
            if self.upload_batch(batch_data):
                success_days += 1
            
            current_date += timedelta(days=1)
            time.sleep(0.1)  # 避免过快请求
            
            if success_days % 5 == 0:
                print(f"用户{user_name}: 已完成{success_days}天数据")
        
        print(f"用户{user_name}完成: {success_days}天数据")
        return success_days
    
    def generate_baselines(self, days=7):
        """生成多天基线"""
        print("开始生成基线数据...")
        success_count = 0
        
        for i in range(days):
            target_date = (datetime.now() - timedelta(days=i+1)).strftime('%Y-%m-%d')
            try:
                response = self.session.post(BASELINE_API, json={'target_date': target_date}, timeout=30)
                if response.status_code == 200:
                    result = response.json()
                    user_count = result.get('user_baseline', {}).get('count', 0)
                    org_count = result.get('org_baseline', {}).get('count', 0)
                    print(f"基线生成成功({target_date}): 用户基线{user_count}条, 组织基线{org_count}条")
                    success_count += 1
                else:
                    print(f"基线生成失败({target_date}): {response.status_code}")
            except Exception as e:
                print(f"基线生成异常({target_date}): {e}")
            time.sleep(1)
        
        return success_count

def main():
    """主函数"""
    simulator = HealthDataSimulator()
    
    print("=== 健康数据模拟器 ===")
    
    # 获取用户（限制数量）
    users = simulator.get_users(limit=3)  # 限制3个用户
    if not users:
        print("没有找到用户数据")
        return
    
    print(f"找到{len(users)}个用户")
    for user in users:
        print(f"- {user['user_name']} ({user['device_sn']})")
    
    # 确认是否继续
    confirm = input("\n是否开始模拟数据上传？(y/N): ")
    if confirm.lower() != 'y':
        print("取消操作")
        return
    
    print("\n开始模拟数据上传...")
    start_time = time.time()
    
    # 并发处理用户数据
    with ThreadPoolExecutor(max_workers=2) as executor:
        futures = {executor.submit(simulator.simulate_user_month, user): user for user in users}
        
        for future in as_completed(futures):
            user = futures[future]
            try:
                days = future.result()
                print(f"用户{user['user_name']}完成: {days}天")
            except Exception as e:
                print(f"用户{user['user_name']}失败: {e}")
    
    elapsed = time.time() - start_time
    print(f"\n数据上传完成!")
    print(f"成功: {simulator.success_count}条")
    print(f"失败: {simulator.error_count}条")
    print(f"耗时: {elapsed:.1f}秒")
    
    # 生成基线
    if simulator.success_count > 0:
        print("\n开始生成基线...")
        baseline_count = simulator.generate_baselines(7)
        print(f"基线生成完成: {baseline_count}天")
    
    print("\n=== 模拟完成 ===")

if __name__ == "__main__":
    main() 