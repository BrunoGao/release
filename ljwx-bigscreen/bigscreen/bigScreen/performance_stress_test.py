...
class PerformanceStressTest:#高性能压力测试
    def __init__(self):
        ...
        self.max_workers=200#最大线程数
        self.timeout=3#请求超时(缩短避免broken pipe)
    ...
    def _init_session_pool(self):#初始化会话池
        for _ in range(self.max_workers):
            ...
            #配置连接池(优化网络稳定性)
            adapter=HTTPAdapter(
                pool_connections=15,#减少连接池大小
                pool_maxsize=15,#减少最大连接数
                max_retries=Retry(total=1,backoff_factor=0.1,status_forcelist=[500,502,503,504]),#启用重试
    ...
    def _save_web_report(self):#保存Web报告(增强错误信息)
...
<head>
    ...
    <title>性能压力测试报告</title>
    <script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
    <style>
        body{{font-family:'Microsoft YaHei',sans-serif;margin:0;padding:20px;background:linear-gradient(135deg,#1e3c72,#2a5298);color:#fff;}}
        .container{{max-width:1400px;margin:0 auto;}}
        .header{{text-align:center;margin-bottom:30px;}}
        .header h1{{color:#64ffda;text-shadow:0 0 20px rgba(100,255,218,0.5);}}
        .summary{{display:grid;grid-template-columns:repeat(auto-fit,minmax(250px,1fr));gap:20px;margin-bottom:30px;}}
        .card{{background:rgba(255,255,255,0.1);backdrop-filter:blur(10px);border:1px solid rgba(100,255,218,0.3);border-radius:12px;padding:20px;box-shadow:0 8px 32px rgba(0,0,0,0.3);}}
        .card h3{{color:#64ffda;margin:0 0 10px 0;}}
        .metric{{font-size:2em;font-weight:bold;color:#fff;text-shadow:0 0 10px rgba(255,255,255,0.5);}}
        .charts{{display:grid;grid-template-columns:1fr 1fr;gap:30px;margin-bottom:30px;}}
        .chart-container{{background:rgba(255,255,255,0.1);border-radius:12px;padding:20px;backdrop-filter:blur(10px);height:400px;position:relative;}}
        .table{{width:100%;border-collapse:collapse;background:rgba(255,255,255,0.1);border-radius:12px;overflow:hidden;}}
        .table th,.table td{{padding:8px;text-align:center;border-bottom:1px solid rgba(255,255,255,0.2);font-size:0.9em;}}
        .table th{{background:rgba(100,255,218,0.2);color:#64ffda;}}
        .status-pass{{color:#4caf50;}}
        .status-fail{{color:#f44336;}}
        .error-summary{{background:rgba(255,152,0,0.1);border:1px solid rgba(255,152,0,0.3);border-radius:8px;padding:15px;margin:20px 0;}}
        .error-summary h4{{color:#ff9800;margin:0 0 10px 0;}}
...
<body>
            ...
            <table class="table">
                ...
                <tbody>
                    {''.join(f'''<tr>
                        ...
                        <td style="font-size:0.8em;">{list(r.get('error_counts',{}).keys())[:2] if r.get('error_counts') else '无'}</td>
    ...
    <script>
        ...
        if(testResults && testResults.length>0){{
            ...
            const deviceData=testResults.map(r=>r.devices);
            ...
            console.log('QPS数据:',qpsData);
            console.log('响应时间数据:',responseData);
            ...
            new Chart(document.getElementById('qpsChart'), {{
                ...
                options: {{
                    ...
                    plugins: {{
                        legend: {{labels: {{color: '#fff',fontSize: 14}}}},
                        title: {{display: true,text: 'QPS性能趋势',color: '#64ffda',fontSize: 16}}
                    }},
                    scales: {{
                        x: {{
                            ticks: {{color: '#fff',fontSize: 12}}, 
                            grid: {{color: 'rgba(255,255,255,0.2)'}},
                            title: {{display: true,text: '测试轮次',color: '#64ffda'}}
                        }},
                        y: {{
                            ticks: {{color: '#fff',fontSize: 12}}, 
                            grid: {{color: 'rgba(255,255,255,0.2)'}},
                            title: {{display: true,text: 'QPS值',color: '#64ffda'}}
                        }}
                    }}
                }}
            }});
            ...
            new Chart(document.getElementById('responseChart'), {{
                ...
                options: {{
                    ...
                    plugins: {{
                        ...
                        title: {{display: true,text: '响应时间分布',color: '#64ffda',fontSize: 16}}
                    }},
                    scales: {{
                        x: {{
                            ticks: {{color: '#fff',fontSize: 11}},
                            grid: {{color: 'rgba(255,255,255,0.2)'}},
                            title: {{display: true,text: '测试轮次',color: '#64ffda'}}
                        }},
                        y: {{
                            ticks: {{color: '#fff',fontSize: 11}},
                            grid: {{color: 'rgba(255,255,255,0.2)'}},
                            title: {{display: true,text: '响应时间(ms)',color: '#64ffda'}}
                        }}
                    }}
                }}
            }});
