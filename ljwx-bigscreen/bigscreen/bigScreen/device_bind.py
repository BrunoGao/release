...
import qrcode, io, base64, hmac, hashlib, time, uuid
...
def get_device_qrcode(sn):
    try:
        ...
        if not device: return jsonify({'code': 404, 'msg': '设备不存在'}), 404
        ...
        url = f"{request.host_url}api/device/bind/apply?sn={sn}&ts={ts}&sign={sign}"
        ...
        return jsonify({'code': 200, 'data': {'qrcode': f"data:image/png;base64,{img_b64}", 'url': url}})
...
def bind_apply():
    if request.method == 'GET':
        ...
        if not all([sn, ts, sign]): return "参数错误", 400
        if int(time.time()) - int(ts) > 3600: return "二维码已过期", 400
        
        secret = current_app.config.get('SECRET_KEY', 'ljwx-secret')
        valid_sign = hmac.new(f"{secret}".encode(), f"{sn}:{ts}".encode(), hashlib.sha256).hexdigest()[:16]
        if sign != valid_sign: return "签名无效", 400
    ...
    try:
        ...
        if device.user_id: return jsonify({'code': 400, 'msg': '设备已绑定'})
        
        existing = DeviceBindRequest.query.filter_by(device_sn=sn, status='PENDING', is_deleted=False).first()
        if existing: return jsonify({'code': 400, 'msg': '已有待审批申请'})
...
def approve_bind():
    ...
    if action not in ['APPROVED', 'REJECTED']: return jsonify({'code': 400, 'msg': '操作无效'})
...
def unbind_device():
    ...
    try:
        device = DeviceInfo.query.filter_by(serial_number=device_sn, is_deleted=False).first()
        if not device: return jsonify({'code': 404, 'msg': '设备不存在'})
        if not device.user_id: return jsonify({'code': 400, 'msg': '设备未绑定'})
...
def get_bind_requests():
    ...
    for req in pagination.items:
        ...
        user = UserInfo.query.filter_by(id=req.user_id).first()
