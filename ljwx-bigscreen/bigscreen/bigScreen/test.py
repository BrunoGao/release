...
# ��建 folium 地图，设置背景颜色为黑色
m = folium.Map(location=[22.54, 114.06], zoom_start=14, tiles=None, control_scale=True)
...
for u, v, key, data in graph.edges(keys=True, data=True):
    ...
    folium.PolyLine(list(zip(x, y)), color="gray", weight=5, opacity=0.8, dash_array='5, 10').add_to(m)

# 读取自定义图标
icon_url = "static/images/blue-icon.png"  # 使用矿井相关的图标
...
# 添加人的位置和健康信息到地图
for i, location in enumerate(people_locations):
    node = nodes.loc[location]
    health_info = health_data[i]
    html = html_template.format(
        image_url="https://heguang-tech-1300607181.cos.ap-shanghai.myqcloud.com/uPic/miner.jpeg",  # 替换为实际图片URL
        name=health_info["name"],
        job=health_info["job"],
        id=health_info["id"],
        phone=health_info["phone"],
        status=health_info["status"],
        heart_rate=health_info["heart_rate"],
        blood_pressure=health_info["blood_pressure"],
        blood_oxygen=health_info["blood_oxygen"],
        co=health_info["co"],
        methane=health_info["methane"],
        oxygen=health_info["oxygen"]
    )
    iframe = IFrame(html, width=350, height=400)
    popup = folium.Popup(iframe, max_width=350)
    folium.Marker(
        location=[node['y'], node['x']],
        popup=popup,
        icon=folium.CustomIcon(icon_url, icon_size=(30, 30))  # 使用自定义图标
    ).add_to(m)

# 保存地图到HTML文件
m.save('mine_topology_map.html')
